...
from utils.calculations import fmt_money, fmt_num, pct_change, selected_month_summary, top_performers, utility_cost_per_usage_breakdown
...
def monthly_takeaways(df, current_month, previous_month, alerts) -> list[str]:
    ...
    takeaways = []
    takeaways.append(
        f"For {cm}, total utility cost was {fmt_money(summary['current']['amount'])}, "
        f"compared with {fmt_money(summary['previous']['amount'])} last month "
        f"({summary['delta_amount']:+.1f}% if prior month is available)."
        if pd.notna(summary['delta_amount']) else
        f"For {cm}, total utility cost was {fmt_money(summary['current']['amount'])}. Prior-month comparison was not available."
    )
    if pd.notna(summary["delta_cpt"]):
        takeaways.append(
            f"Cost per treatment was {fmt_money(summary['current']['cost_per_treatment'], 2)}, "
            f"a {summary['delta_cpt']:+.1f}% change from the previous month."
        )
    takeaways.append(
        f"The dashboard identified {counts['Total']} active anomalies: "
        f"{counts['Critical']} critical and {counts['Review']} review-level items."
    )
    if not alerts.empty:
        top = alerts.iloc[0]
        takeaways.append(
            f"Highest-priority review item: {top['Property']} / {top['Utility']} - {top['Reason']}."
        )
    return takeaways


def report_tables(df, current_month, previous_month, alerts):
    top_anoms = alerts.head(5).copy() if not alerts.empty else pd.DataFrame()
    ...
    return top_anoms, performers, utility_breakdown
...
def generate_monthly_pdf(df, current_month, previous_month) -> bytes:
    ...
    counts = alert_counts(alerts)
    summary = selected_month_summary(df, current_month, previous_month)
    top_anoms, performers, utility_breakdown = report_tables(df, current_month, previous_month, alerts)
    ...
    # Two-column tables: anomalies and performers.
    anomaly_rows = [["Property", "Utility", "Issue", "Impact"]]
    if not top_anoms.empty:
        for _, r in top_anoms.iterrows():
            anomaly_rows.append([
                str(r["Property"])[:22], str(r["Utility"])[:10], str(r["Reason"])[:42], fmt_money(r.get("Estimated Monthly Impact", 0)),
            ])
    else:
    ...
    if not performers.empty:
        for _, r in performers.iterrows():
            ...
            performer_rows.append([
                str(r["property"])[:24], str(r["state"]), fmt_money(r.get("Cost/Treatment"), 2), f"{chg:+.1f}%" if pd.notna(chg) else "N/A",
            ])
    else:
        performer_rows.append(["No performers found", "", "", ""])

    story.append(Paragraph("Top Anomalies", h_style))
    story.append(Table(anomaly_rows, colWidths=[1.65*inch, .75*inch, 3.05*inch, .75*inch], style=_small_table_style()))
    story.append(Spacer(1, 0.05 * inch))
    story.append(Paragraph("Top Performing Properties", h_style))
    story.append(Table(performer_rows, colWidths=[2.6*inch, .65*inch, 1.25*inch, 1.0*inch], style=_small_table_style()))
    story.append(Spacer(1, 0.05 * inch))
    ...
    if not utility_breakdown.empty:
        for _, r in utility_breakdown.head(6).iterrows():
            utility_rows.append([
                str(r["Utility"]), fmt_money(r["This Month"], 3), fmt_money(r["Previous Month"], 3), f"{r['% Change']:+.1f}%" if pd.notna(r["% Change"]) else "N/A", fmt_money(r["Current Cost"]), str(r["Unit"]),
            ])
    else:
    ...
    followups = recommended_followups(alerts)
