...
def load_google_sheet_cached(sheet_id: str, gid: str, cache_buster: str) -> pd.DataFrame:
    """Load a public Google Sheet tab by gid.

    The cache_buster argument is intentionally included so each Streamlit rerun can
    refresh when the 30-second cache expires.
    """
    urls = [
    ...
    for url in urls:
        try:
            ...
            r.raise_for_status()
            text = r.text
            if "<!DOCTYPE html" in text[:200] or "<html" in text[:300].lower():
                raise ValueError("Google returned an HTML page instead of CSV. Check sharing settings.")
            return pd.read_csv(StringIO(text))
        except Exception as e:
            last_error = e

    # Fallback: try full workbook export if the CSV endpoint is picky.
    try:
...
def normalize_data(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["property", "provider", "city", "state", "utility", "unit", "meter", "account"]:
    ...
    out["amount"] = to_number(out["amount"])
    out["usage"] = to_number(out["usage"])
    out["treatments"] = to_number(out["treatments"])
    out["days_billed"] = to_number(out["days_billed"])
    out["billing_date"] = pd.to_datetime(out["billing_date"], errors="coerce")
    out["due_date"] = pd.to_datetime(out["due_date"], errors="coerce")

    # Build a reliable month field.
    if out["billing_date"].notna().any():
        out["billing_month"] = out["billing_date"].dt.to_period("M").dt.to_timestamp()
    else:
        month_num = pd.to_numeric(out["month"], errors="coerce")
        month_names = pd.to_datetime(out["month"].astype(str).str[:3], format="%b", errors="coerce").dt.month
        month_num = month_num.fillna(month_names)
        year_num = pd.to_numeric(out["year"], errors="coerce")
        out["billing_month"] = pd.to_datetime(
            dict(year=year_num.fillna(1900).astype(int), month=month_num.fillna(1).astype(int), day=1),
            errors="coerce",
        )
    ...
    out["usage"] = out["usage"].fillna(0)
    out["treatments"] = out["treatments"].fillna(0)
    out["days_billed"] = out["days_billed"].fillna(0)
...
def available_months(df: pd.DataFrame) -> list[pd.Timestamp]:
    return sorted(df["billing_month"].dropna().unique())
