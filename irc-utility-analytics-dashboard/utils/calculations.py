...
def filter_dimension(df: pd.DataFrame, state="All", property_name="All", utility="All") -> pd.DataFrame:
    out = df.copy()
    if state and state != "All":
        out = out[out["state"] == state]
    if property_name and property_name != "All":
        out = out[out["property"] == property_name]
    if utility and utility != "All":
        out = out[out["utility"] == utility]
    return out


def utility_aggregate(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate by month/property/utility while avoiding treatment double counting within a utility."""
    if df.empty:
...
def portfolio_month_summary(df: pd.DataFrame, month) -> dict:
    """Portfolio metrics for a month. Avoids treatment double-counting across utilities."""
    d = df[df["billing_month"] == pd.to_datetime(month)]
    if d.empty:
        return {"amount": 0, "usage": 0, "treatments": 0, "cost_per_treatment": np.nan, "cost_per_usage": np.nan}
    amount = d["amount"].sum()
    usage = d["usage"].sum()
    # treatments counted once per property/month.
...
def top_performers(df: pd.DataFrame, current_month, previous_month=None, n=5) -> pd.DataFrame:
    ...
    if cur.empty:
        return pd.DataFrame()
    cur_prop = cur.groupby(["property", "state"], as_index=False).agg(amount=("amount", "sum"), treatments=("treatments", "max"))
